...
- `DEVNOTES.md` — session notes, the project's running log (~2300 lines). Read the LAST entry first for current state (it is the authoritative "START HERE" hydration entry; older entries carry stale "hydration" markers and are historical); do not read the whole file. Anchors: three-tier model (~line 927), waller roadmap a–f (~line 1657), combat-walls design spec (~lines 778–920). The "START HERE NEXT SESSION" section (~line 1005) is marked SUPERSEDED — ignore it.
